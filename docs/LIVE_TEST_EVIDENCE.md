...
Produced by `scripts/live-handoff-test.py` against Terminal Handoff Terminal Handoff 1.1.0 (manifest schema 2) on
macOS. Every path below has been replaced with a placeholder; the raw run wrote
its state to a throwaway directory under the system temporary directory and
...
|---|---|---|
| PASS | an unrelated stand-in Claude session is running | pid 13230 |
...
| PASS | the parent was stopped with SIGTERM and no escalation | {"attempts": 1, "enabled": true, "escalates": false, "pid": 13245, "signal": "SIGTERM"} |
...
| PASS | the generation-1 Claude process is no longer running | pid 13245 |
| PASS | the parent's Terminal still has a live shell on ttys021 | 13236 login / 13238 -zsh |
| PASS | the unrelated Claude session was never touched | pid 13230 |
...
| PASS | the parent Claude process is still running | pid 13486 |
